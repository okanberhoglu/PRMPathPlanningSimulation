...
The initial configuration of the planar robot is given in the figure 1. The fixed point of the robot is (0,0) and the end effectors initial position is (3,0). Besides, the length of the links is 1 for each link.
For the results given in this section, the starting configuration is the given initial configuration in the figure 1.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{initial_configuration.png}
    \caption{The initial configuration of the planar robot.}
    \label{fig:initial_configuration}
\end{figure}

First, the algorithm is tested with an easily reachable goal point for the robot. The figures given as figure 2 and figure 3 shows the result of this test. The first figure shows the workspace with the robot and the goal point. The second figure shows the roadmap generated by the algorithm in the configuration space. 
...
\begin{figure}[H]
    ...
    \caption{The roadmap generated by the algorithm in the configuration space of the results shown in figure 2.}
...
\begin{figure}[H]
    ...
    \caption{The roadmap generated by the algorithm in the configuration space of the results shown in figure 4.}
    \label{fig:result_2_c}
\end{figure}

Figure 5 displays the roadmap generated in the configuration space corresponding to the workspace results shown in Figure 4. As demonstrated in Figure 4, the robot successfully reaches the goal configuration within the specified tolerance.
...
\begin{figure}[H]
    ...
    \caption{The roadmap generated by the algorithm in the configuration space of the results shown in figure 6.}
    \label{fig:result_3_c}
\end{figure}

There is a hard end configuration for the robot given in the figure 6. The given goal point is close to robots initial state and there is an obstacle between the robot and the goal point. These conditions makes hard for the robot to reach the goal position.
...
\begin{figure}[H]
    ...
    \caption{The roadmap generated by the algorithm in the configuration space of the results shown in figure 8.}
    \label{fig:result_4_c}
\end{figure}

Figure 8 demonstrates a scenario where the robot cannot reach the specified goal point because it lies outside the robot's reachable workspace. However, the planner successfully generates a path to the reachable configuration that is closest to the desired goal.
...
In Figure 10, the obstacle prevents the robot from reaching the goal point directly. However, the algorithm successfully finds a path to the closest reachable configuration.
...
In Figure 11, the robot cannot reach the goal because it is located far beyond the robot's initial reach. Nevertheless, the algorithm successfully plans a path to the closest reachable point.
...
\begin{figure}[H]
    ...
    \caption{The roadmap generated by the algorithm in the configuration space of the results shown in figure 12.}
    \label{fig:result_7_c}
\end{figure}

Figure 12 demonstrates a scenario where the robot successfully reaches the goal within the specified tolerance. Figure 13 shows the corresponding configuration space roadmap generated by the algorithm.
\section{Conclusion}
